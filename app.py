# app.py (FINAL FIXED VERSION with Context Processor and standardized Endpoints)
...
# Use datetime.date.today() for pure date context
...
def inject_current_year():
    ...
    # Use datetime.datetime.now() for full timestamp access
...
def index():
    ...
    if inventory_list and hasattr(inventory_list[0], sort_by):
        try:
            # Sort Car objects using operator.attrgetter
...
# FIX: The endpoint name is explicitly 'add_car'
...
def add_car():
    ...
    if request.method == 'POST':
        ...
        try:
            # Ensure proper type conversion before validation/use
...
# FIX: The endpoint name is explicitly 'sell_car'
...
# FIX: The endpoint name is explicitly 'edit_car'
    ...
    if request.method == 'POST':
        try:
            ...
            if manager.edit_car(vin, new_price=new_price, new_year=new_year, new_image_url=new_image_url):
                ...
                return redirect(url_for('car_detail', vin=vin)) # Redirect to car_detail endpoint
            else:
...
# FIX: The endpoint name is explicitly 'sales_report'
@app.route('/sales')
def sales_report():
    ...
    # Template expects 'report'
    return render_template('sales.html', report=report) 

# FIX: The endpoint name is explicitly 'car_detail'
@app.route('/car/<vin>')
def car_detail(vin):
